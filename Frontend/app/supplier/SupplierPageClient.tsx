...
export default function SupplierPageClient() {
  ...
  const isTrial = licenseInfo?.LicenseType === "TRIAL" || licenseInfo?.SuppliersAccess === "limited";
  const hasNoAccess = licenseInfo !== null && (licenseInfo?.SuppliersAccess === "none" || (!licenseInfo?.SuppliersAccess && licenseInfo?.LicenseType === "TRIAL"));
  const trialRowLimit = Number(licenseInfo?.SuppliersRowsPerSearch ?? 5);
        ...
        if (Array.isArray(data)) {
          parsedCodes = data
            .map((item: unknown) => {
              ...
              const code = obj.code || obj.hsn_code || obj.hscode || obj.hs_code || obj.HSCode || obj.hsn || obj.id;
              const desc = obj.description || obj.desc || obj.Description || obj.name || "";
  ...
  const fetchSuppliers = async (newPageIndex: number, newPageSize = pageSize) => {
    if (!sessionToken) {
      setToast({ message: "Please login to search suppliers", type: "info" });
      router.push("/login");
      return;
    }
  ...
  const handleRowClick = (supplier: Supplier) => {
    ...
    const searchQuery = `${name} ${countryName} company supplier exporter manufacturer`;
      ...
      <div className="min-h-screen bg-gray-50 pt-24 flex items-center justify-center px-4">
        ...
        >
          <div className="w-16 h-16 rounded-2xl bg-teal-100 flex items-center justify-center mx-auto mb-5">
            <HugeiconsIcon icon={PackageIcon} size={32} className="text-teal-600" />
          </div>
          <h2 className="text-2xl font-bold text-gray-900 mb-3">Unlock Suppliers Search</h2>
          <p className="text-gray-600 mb-6">
            Access to verified global exporters and manufacturers is available on the <strong>Trade</strong> and <strong>Bundle</strong> plans. Upgrade to search by country, HS code, and more.
          </p>
          <a
            href="/plans"
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Header */}
      <div className="bg-linear-to-b from-teal-50/50 to-white border-b border-gray-100 pt-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Search Suppliers</h1>
            <p className="text-gray-600">Find verified exporters and manufacturers by country and HS code</p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="relative" ref={hsCodeRef}>
              ...
              <div className="relative">
                <input
                  ...
                  autoComplete="off"
                  className="w-full h-11 px-4 pr-10 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 transition-all"
                />
                <button
                  type="button"
                    ...
                    >
                      {filteredHsCodes.slice(0, 50).map((item, index) => (
                        <li
                          ...
                          className={`px-4 py-2.5 cursor-pointer hover:bg-teal-50 text-sm transition-colors border-b border-gray-50 last:border-0 ${hsCode === item.code ? "bg-teal-50" : ""
                            }`}
                        >
                          <span className="font-medium text-teal-600">{item.code}</span>
                          {item.description && (
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full h-11 flex items-center justify-center gap-2 text-sm font-semibold rounded-xl text-white disabled:opacity-50 transition-all cursor-pointer disabled:cursor-not-allowed bg-linear-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700"
              >
                {isLoading ? (
                  <>
          ...
          {hsCode && (() => {
            ...
            return selected?.description ? (
              <div className="mt-3 w-full text-xs text-gray-600 bg-gray-50 rounded-xl px-4 py-2.5 border border-gray-100">
                <span className="font-medium text-teal-600">{selected.code}</span>
        ...
        {isTrial && suppliers.length > 0 && (
          <TrialBanner context="suppliers" rowLimit={trialRowLimit} className="mb-4" />
        )}
        ...
        >
          ...
          <div className="px-5 py-4 border-b border-gray-100 flex flex-wrap gap-4 justify-between items-center bg-gray-50">
            <span className="text-gray-600 text-sm">
              <span className="font-semibold text-gray-900">{totalResults.toLocaleString("en-IN")}</span> suppliers found
            </span>
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="text-sm bg-white border border-gray-200 text-gray-700 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-teal-500/20"
              >
                ...
                {isLoading && pageIndex === 1 ? (
                  ...
                  <tr>
                    <td colSpan={7} className="py-20 text-center">
                      <HugeiconsIcon icon={PackageIcon} size={40} className="mx-auto mb-3 text-gray-300" />
                  ...
                  <>
                    {suppliers.map((supplier, index) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 font-medium text-gray-900 group-hover:text-teal-600 transition-colors">
          ...
          {!isLoading && suppliers.length > 0 && (
            <div className="px-5 py-4 border-t border-gray-100 flex flex-wrap gap-4 justify-between items-center bg-gray-50">
              <span className="text-sm text-gray-600">
                Showing {((pageIndex - 1) * pageSize) + 1} - {Math.min(pageIndex * pageSize, totalResults)} of {totalResults.toLocaleString("en-IN")} suppliers
              </span>
              <div className="flex items-center gap-2">
                <button
                ...
                <span className="px-4 py-1.5 text-sm font-medium text-gray-700 bg-teal-50 border border-teal-200 rounded-lg">
