...
export default function BuyerPageClient() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        hsCodeRef.current &&
        !hsCodeRef.current.contains(event.target as Node)
      ) {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        const response = await axios.get(
          `${BACKEND_URL}/supplier-countries`,
          config,
        );
        const countries = Array.isArray(response.data)
          ? response.data.filter(Boolean).sort()
          : [];
      ...
      try {
        const response = await axios.get(
          `${BACKEND_URL}/hscodes-descriptions`,
          config,
        );
  ...
  const fetchAvailableYears = useCallback(
    async (cancelledRef: React.MutableRefObject<boolean>) => {
      ...
      try {
        ...
        if (
          res.status === 403 &&
          (json?.code === "PLAN_NO_ACCESS" || json?.code === "NO_ACCESS")
        ) {
          setToast({
            message:
              json?.error ||
              "Your plan does not include Buyers search. Upgrade to access.",
            type: "warning",
          });
          setAvailableYears([]);
          return;
        ...
        setAvailableYears([]);
        setToast({
          message: "Could not load years. Check HS code and try again.",
          type: "warning",
        });
      } finally {
  ...
  const fetchSuppliers = async (
    ...
    if (validatedHsCode.length < 2) {
      setToast({
        message: "Enter a valid HS code (2–10 digits)",
        type: "warning",
      });
      return;
    }
    ...
    try {
      const validatedPageSize = Math.min(
        validatePageSize(newPageSize, 50),
        100,
      );
      ...
      if (country.trim())
        params.set("country", sanitizeSearchInput(country.trim()));
      ...
      setHasPrev(false);
      setToast({
        message: (e as Error).message || "Failed to load buyers",
        type: "error",
      });
    } finally {
  ...
  const handleCountryChange = (value: string) => {
    ...
    if (sanitized) {
      setFilteredCountries(
        countryData.filter((c) => c.toLowerCase().includes(sanitized)),
      );
    } else {
    ...
    if (sanitized) {
      setFilteredHsCodes(
        hsCodeData
          .filter(
            (item) =>
              item.code.toLowerCase().includes(sanitized) ||
              item.description.toLowerCase().includes(sanitized),
          )
          .slice(0, 100),
      );
    } else {
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          >
            ...
            <p className="text-gray-600">
              Find verified importers by country and HS code with real-time data
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div className="relative">
                ...
                >
                  <HugeiconsIcon
                    ...
                    size={16}
                    className={`transition-transform ${isHsCodeDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
                    ...
                    >
                      {filteredHsCodes.slice(0, 50).map((item, index) => (
                        <li
                          ...
                          className={`px-4 py-2.5 cursor-pointer hover:bg-blue-50 text-sm transition-colors border-b border-gray-50 last:border-0 ${
                            hsCode === item.code ? "bg-blue-50" : ""
                          }`}
                        >
              ...
              <div className="relative">
                ...
                <div className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none">
                  <HugeiconsIcon
                    icon={ArrowDown01Icon}
                    size={16}
                    className={`transition-transform ${isSortByDropdownOpen ? "rotate-180" : ""}`}
                  />
                </div>
            ...
            <div className="relative" ref={yearRef}>
              ...
              <div className="relative">
                ...
                >
                  <span
                    className={!year && !isYearsLoading ? "text-gray-500" : ""}
                  >
                    {isYearsLoading
                      ? "Loading years..."
                      : year === ""
                  ...
                  <HugeiconsIcon
                    icon={ArrowDown01Icon}
                    size={16}
                    className={`text-gray-400 shrink-0 ml-2 transition-transform ${isYearDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
          ...
          {hsCode &&
            (() => {
              const selected = hsCodeData.find(
                (item) => item.code === hsCode.trim(),
              );
          ...
          <div className="overflow-auto max-h-[55vh]">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr>
                  <th className="px-5 py-3 text-left font-semibold text-gray-600 w-16">
                    #
                  </th>
                  ...
                  <>
                    {suppliers.map((supplier, index) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 text-center text-gray-600">
                          {formatNumber(
                            supplier.Frequency ?? supplier.frequency,
                          )}
                        </td>
                        <td className="px-5 py-3.5 text-center text-gray-600">
                          {formatNumber(
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex flex-wrap gap-4 justify-between items-center bg-gray-50">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => handlePageChange(pageIndex + 1)}
                  disabled={
                    (totalPages ? pageIndex >= totalPages : true) || isLoading
                  }
                  className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                  Next
